...
`temporal`, `segments-manager`, `workflows`, `segment-lifecycle-worker`. Each service's
...
Now that `workflows`/`segment-lifecycle-worker` are **Argo-managed**, the `namespaces`
...
- `helm-charts-segment-lifecycle-worker` — extracted from `team-redbull/workflows`'
  `helm/segment-connectivity` (renamed with the domain, then suffixed `-worker`: the
  chart deploys the domain's activity WORKER, not a `segment-lifecycle` service).
...
- `team-redbull/workflows` — Temporal worker code + charts. `helm/workflows` →
  `helm-charts-workflows` and `helm/segment-connectivity` → `helm-charts-segment-lifecycle-worker`
  ...
  **stays here** — it's the only chart still Helmfile-pulled (`git::`), a test-only
  stand-in for the real "next" firewall service `segment-lifecycle-worker` talks to
  (e2e/test environments only, never alongside a production `segment-lifecycle-worker`
  release).
